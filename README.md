...
- [x] Архитектурный паттерн **Уровни изоляции транзакций** (грязное чтение / неповторяющееся чтение / фантомы / потерянное обновление)
- [x] Архитектурный паттерн **Брокеры сообщений** (Kafka pull vs RabbitMQ push)
- [x] Архитектурный паттерн **Классы баз данных** (OLTP/OLAP/HTAP, Persistent/In-memory) — формат карточек-классификации
- [x] Архитектурный паттерн **Виды баз данных** (реляционные/документные/графовые/kv/колоночные/time-series/blob)
...
- [x] Архитектурный паттерн **Стратегии релизов** (Rolling / Blue-Green / Canary)
- [x] Архитектурный паттерн **Паттерны коммуникации микросервисов** (Агрегатор / Цепочка / событийно-ориентированная / отложенное выполнение задач)
- [x] Архитектурный паттерн **Устойчивость к сбоям** (Retries + идемпотентность + backoff, backpressure, graceful degradation / fallback)
- [x] Архитектурный паттерн **Service Discovery и Heartbeat**
- [x] Архитектурный паттерн **CQRS**
- [x] Архитектурный паттерн **Реалтайм-обновления** (Polling / Long Polling / Streaming)
- [x] Архитектурный паттерн **MapReduce** (word count: Cut → Map → Shuffle → Reduce)
- [x] Архитектурный паттерн **Автодополнение: Trie** (сжатое префиксное дерево + пакетное обновление из OLAP)
- [x] Архитектурный паттерн **Throttling / Debouncing** (интерактив в реальном времени)
- [x] Архитектурный паттерн **Консенсус: распределённые транзакции** (2PC / Saga / Transaction Outbox)
- [x] Архитектурный паттерн **Консенсус: блокировки и выбор лидера** (Redis SET NX PX + Bully algorithm)
- [x] Архитектурный паттерн **Функциональные vs нефункциональные требования**
